...
final class CodedInputStreamReader implements Reader {
  ...
  public double readDouble() throws IOException {
    requireWireType(WireFormat.WIRETYPE_FIXED64);
    return input.readDouble();
  }

  @Override
  public float readFloat() throws IOException {
    requireWireType(WireFormat.WIRETYPE_FIXED32);
    return input.readFloat();
  }

  @Override
  public long readUInt64() throws IOException {
    requireWireType(WireFormat.WIRETYPE_VARINT);
  ...
  public String readString() throws IOException {
    requireWireType(WireFormat.WIRETYPE_LENGTH_DELIMITED);
  ...
  public <T> T readGroup(Class<T> clazz, ExtensionRegistryLite extensionRegistry)
      throws IOException {
    requireWireType(WireFormat.WIRETYPE_START_GROUP);
  ...
  private <T> void mergeGroupFieldInternal(
    ...
    endGroupTag = WireFormat.makeTag(WireFormat.getTagFieldNumber(tag), WireFormat.WIRETYPE_END_GROUP);
  ...
  public <K, V> void readMap(
    ...
    try {
      while (true) {
        ...
        if (number == Reader.READ_DONE || input.isAtEnd()) {
          break;
        }
        try {
          switch (number) {
